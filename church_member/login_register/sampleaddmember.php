...
$transfer_sql = "SELECT transferID, tfname, tmname, tlname, tchurchname, ttransferchurch, tplacechurch, transaction_date, tfile AS file, tstatus AS status FROM `transaction-transfer` ";
$baptism_sql = "SELECT baptismID, bfname, bmname, blname, bchurchname, transaction_date, bfile AS file, bstatus AS status FROM `transaction-baptism`";
$excuse_sql = "SELECT excuseID, efname, emname, elname, echurchname, eusedfor, eorganization, transaction_date, efile AS file, estatus AS status FROM `transaction-excuse`";
$purpose_sql = "SELECT purposeID, pfname, pmname, plname, pchurchname, pdescription, transaction_date, pfile AS file, pstatus AS status FROM `transaction-purpose`";
...
if (isset($purpose_result) && mysqli_num_rows($purpose_result) > 0) {
    ...
    while ($row = mysqli_fetch_array($purpose_result)) {
        ...
        echo "<td><button class='opt' onclick='editRow(\"transaction-purpose\", $purposeID)'>Edit</button>" .
...
<div id="editModal" class="modal">
    <div class="modal-content edit-margin"> <span class="close" onclick="closeEditModal()">×</span>
        <h3>Edit Transaction</h3>
        <form id="editForm" onsubmit="submitForm(event)"> <input type="hidden" id="editID" name="id" value="">
            <div class="form-group"> <span id="table"></span> <br> <label>Transaction ID:</label> <span
                    id="transactionID"></span><br> <label>Status:</label><br>
                <div class="form-check form-check-inline"> <input class="form-check-input" type="checkbox"
                        id="editTransferStatus" name="status" value="pending"> <label class="form-check-label"
                        for="editTransferStatus">Pending</label>
                </div>
                <div class="form-check form-check-inline"> <input class="form-check-input" type="checkbox"
                        id="editTransferStatus" name="status" value="progress"> <label class="form-check-label"
                        for="editTransferStatus">In Progress</label> </div>
                <div class="form-check form-check-inline"> <input class="form-check-input" type="checkbox"
                        id="editTransferStatus" name="status" value="done"> <label class="form-check-label"
                        for="editTransferStatus">Done</label> </div>
            </div> <button type="submit" class="btn btn-success" id="saveChangesButton" onclick="submitForm(event)">Save
                Changes</button> <button type=" button" class="btn btn-secondary" id="closeButton"
...
<script>
    ...
    // Set the tableName and transactionID to display in the modal
    function editRow(tableName, transactionID) {
        openEditModal();
        ...
        // Set the status checkbox based on the current row's data-status value
        var status = document.querySelector("[data-status='" + transactionID + "']");
        if (status) {
            var checkboxes = document.getElementsByName("status");
            checkboxes.forEach(function (checkbox) {
                checkbox.checked = false;
            });
            var checkbox = document.getElementById("editTransferStatus" + status.value.capitalize());
            if (checkbox) {
    ...
    function submitForm(event) {
        ...
        var url = "editTransaction.php"; // replace with your update script URL
