...
<body>
    ...
    <div class="contents">
      <form action="" method="POST">
        ...
        <div>
          ...
          <input type="text" id="ministerID" name="ministerID" class="data-insert" placeholder="Enter Minister ID"
            onkeyup="ministerIDKeyUp(this.value)" required>
          <br>
        </div>
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    echo "<tr>";
    echo "<th><a href=\"ministerassigned.php?sort=ministerID&order=$order\">ID</a></th>";
    echo "<th><a href=\"ministerassigned.php?sort=districtname&order=$order\">Districtname</a></th>";
    echo "<th><a href=\"ministerassigned.php?sort=year&order=$order\">Year</a></th>";
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      echo "<tr>";
      echo "<td class='minister-id' data-ministerid='" . $row['ministerID'] . "'>" . $row['ministerID'] . "</td>";
      ...
      echo "<td>
        ...
        <a href='ministerassigned.php?delete=true&ministerID=" . $row['ministerID'] . "&year=" . $row['year'] . "' class='opt1' onclick='return confirm(\"Are you sure you want to delete this record? ID:" . $row['ministerID'] . " Year:" . $row['year'] . "\")'>Delete</a>
  ...
  <script>
    $(document).ready(function () {
      $('.minister-id').hover(function () {
        var ministerID = $(this).data('ministerid');
        $.ajax({
          url: 'getMinisterName.php',
          type: 'post',
          data: { ministerID: ministerID },
          dataType: 'json',
          success: function (data) {
            if (data.ministerfname && data.ministerlname) {
              $(this).append('<div class="minister-popup">' + data.ministerfname + " " + data.ministerlname + '</div>');
            } else {
              $(this).append('<div class="minister-popup">Minister not found</div>');
            }
          }.bind(this)
        });
      }, function () {
        $('.minister-popup').remove();
      });
    });
  </script>
